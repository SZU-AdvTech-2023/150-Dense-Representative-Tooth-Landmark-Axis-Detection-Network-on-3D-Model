...
def get_dists(points1, points2):
    ...
    B, M, C = points1.shape
    _, N, _ = points2.shape
    dists = torch.sum(torch.pow(points1, 2), dim=-1).view(B, M, 1) + torch.sum(torch.pow(points2, 2), dim=-1).view(B, 1, N)
...
class Loss(nn.Module):
    ...
    def forward(self, x, pred):
        x = x.permute(0, 2, 1)
        loss1 = self.mse_loss(x[:, 0, :], pred[:, 0, :])
        loss2 = self.mse_loss(x[:, 1, :], pred[:, 1, :])
        loss3 = self.mse_loss(x[:, 2, :], pred[:, 2, :])
        return loss1, loss2, loss3


if __name__ == "__main__":
    teeth_points = torch.randn(2, 4096, 3).cuda()
    teeth_normals = torch.randn(2, 4096, 3).cuda()
